...
- `smart_exists` / `smart_isdir` no longer report a remote failure as a plain miss. On a negative result they re-probe with `info()`, which propagates, and log the real exception (wrong endpoint, refused connection, missing credential) instead of leaving the caller to print "not found". The probe logs at DEBUG in the generic helpers — they also probe *optional* sidecars on successful loads, and S3 answers a missing-key HEAD with 403 when `s3:ListBucket` is denied — and at ERROR in `smart_database_exists`, the one place the caller is about to hard-fail.

- Simplified datasets are now pruned by region *before* any file is read. `_load_dataset` scheduled every file in the dataset and clipped rows afterwards, so a 1-degree query against a global dataset read all of it — measured on a real 12,461-partition GEDI dataset, the ROI `-51,0,5,1` can only be held by **25** partitions, and `-111.5,52.5,-110.5,53` by **4**. Selection costs ~2 s of driver CPU and no I/O. `_select_dataset_files` derives candidate partition IDs from the filenames and intersects them with the region: H3 via `intersect_h3_geometries` with its `expand_ring=1` overhang guard, EGI by intersecting the partition squares themselves (a nested axis-aligned grid, so containment makes this exact; the region is densified before reprojection — EPSG:6933 bends straight lon/lat edges, and an un-densified sparse-vertex polygon's chord can cut hundreds of km inside the true boundary — and the 1 m buffer absorbs `to_hash` float-boundary rounding). Every query CLI inherits it — `gh3_aggregate`, `gh3_extract`, `gh3_rasterize`, `gh3_from_img`, `gh3_from_polygon` all funnel through `gh3_load` / `egi_load`. The H3 *database* path already worked this way; this closes the gap for datasets, including EGI datasets, which `egi_load` delegates to the same function.
  Pruning only engages when it is provably safe: the dataset sidecar must declare a partition level *strictly coarser* than the index level, all basenames must sit at that level, and none may post-date the sidecar's `partition_ids` (`_dataset_prune_is_safe`). This matters because `gh3_export_part` can name a file after its first row's cell when a sidecar-less source collapses the partition level onto the index level — such names describe *some* rows, not all, and pruning on them would silently drop the rest. Everything unprovable — no sidecar, equal levels, renamed files, hive-style trees, hand-named files — reads the full list; degenerate EGI squares are kept rather than decided against; an empty intersection keeps one file so the schema read and dask graph stay well-formed while the clip yields the correct empty result. Row-level clipping is unchanged.
- Eager dataset loads via the internal `_load_dataset(..., lazy=False)` honour `region` (they returned the full, unclipped frame; note `gh3_load(lazy=False)` was never affected — it computes the lazy path, which always clipped). Both eager and lazy paths now warn instead of clipping when the dataset has no geometry column — the lazy path previously fell through to dask's *numeric* `DataFrame.clip(lower=region)` on non-geo data. `region` also accepts the CLI's `"W,S,E,N"` string form everywhere, via `region_to_geometry`.

### Added
- `utils.region_to_geometry`: the single normalizer for every accepted region form (vector path, `"W,S,E,N"`, `[W,S,E,N]`, GeoDataFrame/GeoSeries, shapely geometry) → one EPSG:4326 geometry. Extracted from `intersect_h3_geometries` and now shared with EGI partition selection, so the two index types cannot drift apart on region handling.
...
- `s3://host:port/bucket/...` URLs configure the endpoint themselves, so a self-hosted server no longer has to be named twice. A bucket name cannot contain `:`, so a netloc with an explicit port is unambiguously a host; port 443 implies https, anything else http. `--s3-endpoint` still wins (and now also accepts the `s3://host:port` spelling, normalized by the same port rule instead of being mangled), the `s3_endpoint` argument itself is never rewritten by the URL scan, a host URL with no bucket path is rejected with a clear error, and a default database path from `~/.gedih3.env` gets the same normalization as an explicit `-d` (`cliutils.endpoint_from_s3_urls` + `configure_database_path`).
...
- Remote parquet reads now transfer the column chunks and nothing else. Two layers were over-fetching: fsspec pulls a 5 MB block around every range its cache sees, and `pandas.read_parquet` does not coalesce the scattered column-chunk reads pyarrow is capable of batching. Measured against `rclone serve http` on a 1.8 GB partition, projecting one column (17.7 MB of chunks + 0.33 MB footer): **359 MB → 18.1 MB** of server-side reads, 50 → 26 opens. `utils.smart_open_columnar` opens remote files with `cache_type='none'`; `utils.read_parquet_coalesced` reads them with `pre_buffer=True` and `use_pandas_metadata=True` (so a column-projected read keeps the frame's named index, exactly as `pandas.read_parquet` does), routing the non-geo case through `pyarrow.parquet.read_table` because pandas' reader ignores `pre_buffer`. Wired into `cliutils.make_dataset_reader`, `gh3driver._read_parquet_files`, all three strategies of `gh3driver._read_parquet_bbox`, and the three remote footer-only `pq.read_schema` sites. Local reads are untouched.
- `gh3_rasterize` works on remote datasets. It opened the dataset sidecar with the builtin `open()` and derived the H3 partition level with `os.listdir` — neither of which can see an `s3://` or `http://` dataset. The level now comes from `h3_partition_level` in the sidecar (a-priori), falling back to `partition_ids`, then a manifest-backed `smart_glob`; on local datasets the sidecar value is cross-checked against one filename (ground truth) and the filename wins with a warning when they disagree.
- `gh3_doctor` rejects remote roots (`-i`, `--soc-dir`, `--tmpdir`) with a clear message instead of `os.path.abspath`-ing the URL into a nonsense local path (`/cwd/http:/host:8898/db`). Every diagnosis walks the tree with `os.scandir` on workers and `--fix` rewrites files in place, so it stays a local tool by design.
- `utils.json_write` drops its path from the `json_read_cached` cache, making read-after-write coherence structural rather than stat-dependent; `json_read_cached`'s docstring now states the returned object is the shared cache entry and must not be mutated.
- Region handling now propagates uniformly through the Python API, the CLIs and the DuckDB pre-query selector — every selection path routes through `region_to_geometry`:
  - `egi_load` / `_prepare_egi_loading` accept the full region set (`vector path`, `"W,S,E,N"`, `[W,S,E,N]`, GeoDataFrame/GeoSeries in any CRS, shapely) instead of only GeoDataFrame/bbox, and always normalize to EPSG:4326 — a projected or CRS-naive GeoDataFrame no longer reaches the clip stage unconverted.
  - `gh3_load`'s H3-database clip replaced its ad-hoc list/GeoSeries/else normalizer with `region_to_geometry`, so a region that selects partitions is always clippable (string forms included). CRS-naive geopandas input is assumed EPSG:4326 package-wide.
  - `gh3_select_partitions` answers for simplified datasets too (H3 and EGI), through the same safety-gated `_select_dataset_files` machinery the loaders use — with an empty list (not a schema placeholder file) when nothing intersects, and the conservative full list when the sidecar cannot prove the filenames bound their files.
  - `sqlutils.geoseries_to_cells` / `geoseries_to_filter` (the DuckDB pre-query partition filter) accept the same region forms and normalize CRS — a projected GeoSeries used to be fed to h3 as if it were lat/lng degrees — and the SQL filter column now follows the `resolution` argument (`h3_04 = ANY(...)` for resolution 4) instead of always emitting `h3_03`.
- `egi.aoi_tiles` selects region tiles via the spatial index instead of a serial per-tile `.apply` over all 19,656 global level-12 tiles (~1.5 s -> ms per query), and densifies the region before reprojecting to EPSG:6933 (same sparse-vertex chord hazard as the dataset pruning fix; 3 of 37 centerline tiles were dropped for a 60-degree quad pre-fix).
- `gh3_build_ducklake` lists parquet files via the manifest-aware `smart_glob` (`_manifest.txt` sentinel, O(1) on the GPFS metadata server) instead of guessing filenames from a hardcoded year range and the `<hex>.<year>.0.parquet` naming contract — the real listing also catches fragments the guess missed.
