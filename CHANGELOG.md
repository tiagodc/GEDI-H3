...
- Simplified datasets are now pruned by region *before* any file is read. `_load_dataset` scheduled every file in the dataset and clipped rows afterwards, so a 1-degree query against a global dataset read all of it — measured on a real 12,461-partition GEDI dataset, the ROI `-51,0,5,1` can only be held by **25** partitions, and `-111.5,52.5,-110.5,53` by **4**. Selection costs ~2 s of driver CPU and no I/O. `_select_dataset_files` derives candidate partition IDs from the filenames (which *are* the partition IDs — `h3_write_dataset_metadata` builds `partition_ids` from them, so this survives a stale sidecar and works on datasets written by older versions) and intersects them with the region: H3 via `intersect_h3_geometries` with its `expand_ring=1` overhang guard, EGI by intersecting the partition squares themselves (a nested axis-aligned grid, so containment makes this exact; the 1 m buffer only absorbs `to_hash` float-boundary rounding). Every query CLI inherits it — `gh3_aggregate`, `gh3_extract`, `gh3_rasterize`, `gh3_from_img`, `gh3_from_polygon` all funnel through `gh3_load` / `egi_load`. The H3 *database* path already worked this way; this closes the gap for datasets, including EGI datasets, which `egi_load` delegates to the same function.
  Under-selection would be silent data loss, so the fallbacks all lean the other way: filenames that are not partition IDs (hive-style trees, hand-named files) read everything; degenerate EGI squares are kept rather than decided against; an empty intersection keeps one file so the schema read and dask graph stay well-formed while the clip yields the correct empty result. Row-level clipping is unchanged.
- Eager dataset loads honour `region`. `gh3_load(..., region=..., lazy=False)` on a simplified dataset never applied the spatial filter at all — it returned the full, unclipped frame. It now clips like the lazy path, and warns instead of clipping when the dataset has no geometry column.
...
- Remote parquet reads now transfer the column chunks and nothing else. Two layers were over-fetching: fsspec pulls a 5 MB block around every range its cache sees, and `pandas.read_parquet` does not coalesce the scattered column-chunk reads pyarrow is capable of batching. Measured against `rclone serve http` on a 1.8 GB partition, projecting one column (17.7 MB of chunks + 0.33 MB footer): **359 MB → 18.1 MB** of server-side reads, 50 → 26 opens. `utils.smart_open_columnar` opens remote files with `cache_type='none'`; `utils.read_parquet_coalesced` reads them with `pre_buffer=True`, routing the non-geo case through `pyarrow.parquet.read_table` (verified to produce an identical frame, dtypes included) because pandas' reader ignores it. Wired into `cliutils.make_dataset_reader`, `gh3driver._read_parquet_files` and all three strategies of `gh3driver._read_parquet_bbox`. Local reads are untouched.
