...
def _make_synthetic_raster(
    path, nx=10, ny=10, crs="EPSG:4326", bounds=(-10, -10, 10, 10), values=None, nbands=1, nodata=None
):
    ...
    if nbands > 1:
        ...
        data = xr.DataArray(values, dims=["band", "y", "x"], coords={"band": np.arange(1, nbands + 1), "y": y, "x": x})
    else:
        if values.ndim == 3:
            values = values[0]
        data = xr.DataArray(
...
class TestResolveRasterSource:
    ...
    def test_directory_multiple_tiles_builds_vrt(self, tmp_dir):
        for i in range(3):
            _make_synthetic_raster(os.path.join(tmp_dir, f"tile_{i}.tif"), bounds=(i * 10, 0, (i + 1) * 10, 10))
...
class TestSamplingWithWindows:
    def test_window_mean_in_sampling(self, tmp_dir):
        ...
        result = sample_raster_at_points(pts, path, band_names=["val"], window_ops=window_ops)
...
class TestCRSReprojection:
    def test_utm_raster_wgs84_points(self, tmp_dir):
        ...
        values = np.arange(100, dtype=float).reshape(10, 10)
        _make_synthetic_raster(path, values=values, crs="EPSG:32633", bounds=(450000, 5500000, 460000, 5510000))
...
class TestComputeSamplingMeta:
    def test_basic_meta(self):
        meta = _compute_sampling_meta(band_names=["b0", "b1"], window_ops=None, geo=False, partition_col="h3_03")
    ...
    def test_meta_with_windows(self):
        ...
        meta = _compute_sampling_meta(band_names=["elevation"], window_ops=wops, geo=False, partition_col=None)
        window_cols = [c for c in meta.columns if "mean_3x3" in c]
        assert len(window_cols) == 1

    def test_meta_geo(self):
        meta = _compute_sampling_meta(band_names=["val"], window_ops=None, geo=True, partition_col=None)
...
class TestIntegration:
    ...
    def test_h3_database_sampling(self, tmp_dir):
        ...
        from gedih3.cliutils import h3_col_name, parse_region

        region = parse_region(self.REGION)
        from shapely.geometry import box as shapely_box

        # Use Romania bounds intersected with DEM
        img_gdf = gpd.GeoDataFrame(geometry=[shapely_box(*info["bounds_wgs84"])], crs="EPSG:4326")
        roi = gpd.overlay(img_gdf, region.to_crs("EPSG:4326"), how="intersection")

        with Client(n_workers=2, threads_per_worker=1, memory_limit="4GB") as _client:
            ddf = gh3.gh3_load(columns=["geometry"], region=roi, gh3_dir=self.H3_DB)
            ...
            if len(sample_part) > 0:
                ...
                result = sample_raster_at_points(
                    sample_part, raster_path, band_names=info["band_names"], partition_col=partition_col
                )
