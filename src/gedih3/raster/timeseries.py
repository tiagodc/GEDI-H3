...
class TimeSeriesRasterizer:
    ...
    def _rasterize(self, gdf: gpd.GeoDataFrame) -> xr.Dataset:
        ...
        if self.use_egi:
            ...
            # A time-window aggregate over an arbitrary ROI legitimately spans
            # multiple 160-km outer tiles — so split explicitly and merge.
            # rasterize_partition does NOT split: a Dask partition nests in one
            # outer tile by construction, and it reports anything else as the
            # invariant violation it is. This caller is not a partition.
            rasters = []
            for _, tile_gdf in egi.split_by_outer_tile(gdf):
                rasters.extend(egi.rasterize_partition(tile_gdf, columns=self.columns))
            if len(rasters) == 0:
                return xr.Dataset()
            if len(rasters) == 1:
                return rasters[0]
            return egi.merge_raster_partitions(object_series(rasters))
        else:
