...
def gh3_load_part(h3_part_id, h3_product_vars, query=None, h3_res_level=12, gh3_dir=GH3_DEFAULT_H3_DIR):
    df = None
    ...
    df = df.set_index(f'h3_{h3_res_level:02d}')
...
def gh3_load_all(h3_product_vars, region=None, query=None, h3_res_level=12, gh3_dir=GH3_DEFAULT_H3_DIR): 
    h3_ids = gh3_list_parts(gh3_root_dir=gh3_dir)
    
    if region is not None:
        h3_ids = intersect_h3_geometries(region, h3_ids=h3_ids)
        
    return dask.dataframe.from_map(gh3_load_part, h3_ids, h3_product_vars=h3_product_vars, query=query, h3_res_level=h3_res_level, gh3_dir=gh3_dir)