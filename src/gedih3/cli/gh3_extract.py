...
def get_cmd_args():
    ...
    p = argparse.ArgumentParser(description="Extract and filter GEDI shots with H3 or EGI spatial indexing")

    # Database/output configuration
    p.add_argument("-d", "--database", dest="database", type=str, default=None, help="path to H3 database directory")
    ...
    # Options
    p.add_argument("-g", "--geo", dest="geo", action="store_true", help="export as georeferenced points")
    p.add_argument("-t", "--time", dest="add_datetime", action="store_true", help="add datetime column to output")
...
def main():
    ...
    with cli_exception_handler(args):
        ...
        if len(columns) > 0:
            ...
            raise ValueError(
                "No variables selected for extraction. Please specify variables with -l/--list or product-specific options."
            )
        ...
        with Client(**dask_kwargs) as client:
            ...
            if use_egi:
                ...
                if args.egi_shuffle:
                    ...
                    ddf_h3 = gh3.gh3_load(columns=columns, region=region, query=query_str, gh3_dir=args.database)
                    logger.info(f"  Loaded {ddf_h3.npartitions} H3 partitions")
                    logger.info("  Converting to EGI (shuffling data)...")
                    ddf = gh3.egi_extract(
                        ddf_h3, index_level=egi_index_level, partition_level=egi_partition_level, add_geometry=True
                    )
                else:
                ...
                ddf = gh3.gh3_load(columns=columns, region=region, query=query_str, gh3_dir=args.database)
