...
def soc_file_tree(
    ...
    if isinstance(file_struct, str) and os.path.isdir(file_struct):
        ...
        # Always walk the SOC tree on the dask cluster. We intentionally do
        # not consume ``_soc_manifest.txt`` here: external population paths
        # (manual rsync, NASA delivery) bypass the producer-driven refresh
        # and would leave the manifest stale relative to disk, silently
        # narrowing every downstream scan. ``walk_soc_parallel`` fans the
        # year/doy scandir across the registered dask Client, so the cost
        # of always walking is bounded.
        from .parallel import walk_soc_parallel
