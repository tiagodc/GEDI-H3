...
| `_select_dataset_files(data_files, region)` | `gh3driver.py` | Region → dataset file subset, for BOTH index types (H3 via `intersect_h3_geometries` ring-1; EGI via partition-square intersection). Candidates come from the filenames, which *are* the partition IDs. Applied in both branches of `_load_dataset`, so every query CLI inherits it. Fails safe: unparseable names → all files; empty intersection → keep one file for schema/meta. |
