...
- **One GEDI release per database, v3 by default**: `GEDI_DEFAULT_VERSION = 3` (`config.py`) is the only `version=None` fallback; every product (L1B/L2A/L2B/L4A/L4C) of a build or download comes from the same release and `--gedi-version` applies to all of them. Resolution order: explicit `--gedi-version` > build log `gedi_version` (resume; a contradicting arg raises) > `resolve_soc_version(soc_dir)` (download log, else first `_V00N` filename — fresh local builds and resumed/external downloads) > package default. ORNL DAAC identifiers are version-pinned dicts (`{2.1: …_V2_1_2056, 3: …_V3_2508}` / `{2: …_2338, 3: …_V3_2520}`) resolved by `_resolve_identifier`; LP DAAC uses one short_name plus a zero-padded CMR `version` filter that `search_data` always attaches.
