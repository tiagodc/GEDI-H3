...
- **Variable-only update: per-(cell, year) tasks with driver-side SOC broadcast** (v0.10.21+): `_build_add_variables` shards work the same way the fresh-build merge phase does — one task per `<cell>/year=YYYY/*.parquet`. The SOC tree (`orb_track → {prod: path}`) is built ONCE on the driver via `soc_file_tree(...)` and `client.scatter(all_soc, broadcast=True)`'d to every worker. The previous per-cell worker called `soc_file_tree` itself, which after the SOC manifest read-removal fans `walk_soc_parallel` back to the same cluster from inside a worker — deadlock under any workers-equal-threads config. Per-(cell, year) granularity also bounds per-task memory to one year's `new_vars_df`, makes the resume schema-check trivially per-file, and exposes finer-grained parallelism to dask. A Phase 2 follow-up fans `client.map(h3_merge_metadata, touched_cells)` to aggregate the per-year sidecars into per-cell `<cell>.metadata.json`. Atomicity: `parquet_join_columns` writes via `.join.tmp` + `os.replace` and filters columns already present in the base file, so re-running against a partially-updated year file never duplicates columns.
