...
3. **Assigns** each shot an H3 spatial index at a fine resolution (default: level 12, ~25 m — roughly the GEDI footprint size)
4. **Groups** shots into spatial partitions at a coarser H3 resolution (default: level 3, ~12,000 km²)
5. **Writes** column-oriented GeoParquet files — one per spatial partition — along with a build log that records exactly what was built and how
...
│   └── data.parquet
├── h3_03=8035fffffffffff/
│   └── data.parquet
│   ...
└── gedih3_build_log.json
```

Each directory is named after an H3 level-3 cell (the partition key). Inside is a single `data.parquet` file containing all shots whose fine-grained H3 index falls within that partition cell. The `gedih3_build_log.json` at the root records the full build configuration — products, variables, region, temporal range, and H3 resolution settings.
...
```python
# Read a partition directly with pandas — no gedih3 required
import pandas as pd
df = pd.read_parquet('~/gedih3_db/h3/h3_03=8031fffffffffff/data.parquet')
```
...
| `cover_z` (array, 19 elements) | L2B | `cover_z_000_l2b` … `cover_z_018_l2b` (19 columns) |
| `pavd_z` (array, 19 elements) | L2B | `pavd_z_000_l2b` … `pavd_z_018_l2b` (19 columns) |
| `rxwaveform` (array, 1420 bins) | L1B | `rxwaveform_0000_l1b` … `rxwaveform_1419_l1b` (1420 columns) |
| `wsci` | L4C | `wsci_l4c` |
...
**Array variable expansion:** Array variables in GEDI HDF5 files are expanded into individual named columns using zero-padded indices. The index width depends on the variable type: 3 digits for ecological arrays (`rh`, `cover_z`, `pavd_z`) and 4 digits for waveforms (`rxwaveform`).

- `rh` (101 elements per shot) → `rh_000_l2a` through `rh_100_l2a`
- `cover_z` (19 height bins) → `cover_z_000_l2b` through `cover_z_018_l2b`
- `pavd_z` (19 height bins) → `pavd_z_000_l2b` through `pavd_z_018_l2b`
- `rxwaveform` (1420 bins) → `rxwaveform_0000_l1b` through `rxwaveform_1419_l1b`
...
> **Variable subsetting is the most impactful build-time decision.** Raw GEDI HDF5 files are large (1–3 GB each), and variables you include become permanent columns in every downstream query. More columns = larger files, slower queries, longer builds. Two rules of thumb:
> - **Always use `default` or an explicit variable list.** Never use `all`/`*` unless you have a specific reason — L2A `all` alone exceeds 300 variables, many of them diagnostic outputs with limited research value.
...
Each GEDI product has its own flag:
...
| `-l4c` | L4C (structural complexity, WSCI) |
...
**`minimal`** — the smallest usable set. Geolocation, timestamp, quality flag, and the primary headline variable for each product. Use this when disk space is very limited or you only need one metric.

```bash
gh3_build -r "-51,0,-50,1" -l2a minimal -l4a minimal
```

**`default`** — the recommended science-ready set. An expert-curated selection covering all variables needed for common research workflows, including uncertainty estimates, alternative algorithm outputs, and land-cover ancillary data. This is the right choice for most projects.

```bash
gh3_build -r "-51,0,-50,1" -l2a default -l4a default
```

**Explicit variable names** — list specific HDF5 variable names after the flag. Use `gh3_list_variables` to browse available names.

```bash
gh3_build -r "-51,0,-50,1" -l2a rh elev_lowestmode quality_flag -l4a agbd agbd_se l4_quality_flag
```

**`all` or `*`** — every variable in the product. Produces very large databases. L2A alone has over 300 variables. Use with caution and only if you have a specific reason to need the full variable set.
...
> See [Variable Presets Reference](../concepts/variable-presets.md) for the exact variable list in each preset for all products and versions.
...
**`-h3r INDEX_LEVEL`** (default: `12`) — the H3 resolution at which each shot is indexed. Each shot gets assigned to the H3 cell at this level that contains its coordinates. Level 12 cells are ~307 m² — approximately the size of a GEDI footprint, so each footprint typically falls in its own cell.
...
- **Smaller region**: Increase `-h3p` (e.g., to 5 or 6) to get fewer, more evenly filled partition files. With the default level-3 partitions, a small bounding box may produce many nearly-empty partition files.
...
The default mode reads HDF5 files that you have already downloaded to disk. The expected layout is the SOC (Science Operation Center) directory structure: `soc/LXXX/year/doy/*.h5`.
...
With `-s3`, `gh3_build` streams GEDI data directly from NASA's S3 bucket into a temporary location and converts it to Parquet without ever writing the full HDF5 files to persistent storage. Each granule is streamed, processed, and discarded.
...
**Best for**: environments with slow or expensive local disk, cloud computing instances, or any situation where you want to avoid storing the raw HDF5 files (~1–3 GB per granule). S3 mode requires a good network connection to NASA's servers. NASA Earthdata credentials must be configured.

> **S3 mode requires Earthdata credentials.** If you have not already authenticated, run `python -c "import earthaccess; earthaccess.login()"` and follow the prompts. Credentials are stored in `~/.netrc`.
...
Pass a region specification to restrict the build to shots within your area of interest. Only granules that intersect the region are downloaded/processed, and only shots inside the region are written to the database.
...
gh3_build -r BRA -l2a default -l4a default
```
...
GEDI has been collecting data since April 2019. Without temporal subsetting, the build will include all available data — which grows every year.
...
|--------------------|-------------------------------------|
| `minimal` (any product) | ~5–10 GB |
| `default` (any product) | ~40–80 GB |
| `all` (L2A, ~300 vars) | ~300–600 GB |
| `rxwaveform` (L1B, 1420 cols) | ~1,000+ GB — avoid for large areas |

If you are not sure which variables you will need, **`default` is the right starting point** — it covers the variables needed for the vast majority of research workflows. You can always add variables later (see Resume and Updates below) without rebuilding from scratch.
...
| `-T` | 2 | Threads per worker |
| `-M` | auto | Memory per worker (e.g., `8GB`) |
...
```bash
gh3_build -r ... -l4a default -N 8 -T 2 -M 8GB
```
...
```bash
# Use the aggressive memory config to prevent accumulation
gh3_build -r ... -l4a default -N 8 -M 16GB --dask-config dask-config-aggressive-memory.yaml
```

The aggressive memory config enables automatic worker restarts on a schedule, preventing PyArrow's unmanaged memory from accumulating over long builds.

---

## Verifying the Result

After building, use `gh3_read_schema` to inspect the database and confirm it contains what you expect:

```bash
gh3_read_schema ~/gedih3_db/h3/gedih3_build_log.json
```

This prints the full build configuration: products built, variable lists, region, temporal range, H3 index and partition levels, build date, and completion status. Check this before running `gh3_extract` or `gh3_aggregate` to confirm the database contains the variables and region you need.

You can also inspect any individual partition file:

```bash
gh3_read_schema ~/gedih3_db/h3/h3_03=8031fffffffffff/data.parquet
```
...
gh3_build -r large_region.shp -l2a default -l4a default \
          -s3 -N 32 -M 16GB --dask-config dask-config-aggressive-memory.yaml
